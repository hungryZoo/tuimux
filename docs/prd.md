...
- Cmd-Shift-X는 선택 텍스트를 system clipboard에 복사한 뒤 tuimux selection을 해제한다.
...
- macOS PTY UI smoke에서 drag selection, right-click context menu Cut/Copy, Ctrl-C clipboard copy, foreground child SIGINT 미전달, context menu Paste, child bracketed paste wrapper 보존이 통과한다.
