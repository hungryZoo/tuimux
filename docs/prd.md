...
v0.3 UX 방향은 “기능을 많이 보이는 화면”이 아니라 **중앙의 tmux 작업 영역 + 우측의 최소 조작 영역**이다. 이전 초안의 좌측 파일 탐색기, 하단 메뉴 바, 우측 PROCS 패널은 MVP에서 제외한다.
...
- `tuimux --layout-preview`와 기본 TUI 실행 화면이 v0.3 레이아웃을 일관되게 보여준다.
...
- `--help`, `--version`, `--doctor`.
- `--layout-preview`에서 v0.3 레이아웃 출력.
- 기본 TUI에서 v0.3 레이아웃 렌더.
...
- **M0 — 스켈레톤**: 설치, preview, doctor, v0.3 layout, hover/dialog scaffold.
