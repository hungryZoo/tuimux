...
tuimux는 prefix를 외우지 않고 mouse-first로 다룰 수 있는 terminal multiplexer다. v0.2.0-alpha.10의 기본 실행 경로는 tmux wrapper가 아니라 Rust-native daemon-backed multiplexer이며, 세션/윈도우/nested pane tree/PTY를 tuimux daemon이 직접 소유한다.
...
- pane split이 외부 tmux에 맡겨져 있으면 tuimux가 mouse 좌표, selection, clipboard UX를 끝까지 통제하기 어렵다.
...
- 열린 client가 있는 상태에서 두 번째 client가 snapshot/command를 수행하고 세 번째 client가 shutdown할 수 있다.
...
다음 큰 제품 단계는 tmux layout string 호환성, daemon restart persistence, Linux/Windows backend, client별 독립 viewport/cursor 정책, 더 넓은 terminal app compatibility suite다.
