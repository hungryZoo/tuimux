...
- 선택 영역이 있을 때 Ctrl-C/Ctrl-Shift-C/Cmd-Shift-C는 child process SIGINT가 아니라 system clipboard copy로 처리된다. Cmd-Shift-X는 editable 선택 텍스트를 clipboard에 복사한 뒤 cursor 이동과 Backspace로 삭제하고, 삭제할 수 없는 출력 화면 선택은 복사 후 tuimux selection을 해제한다. 선택 영역이 없으면 plain Ctrl-C는 child process interrupt로 전달된다.
...
- Ctrl-V/Ctrl-Shift-V/Cmd-Shift-V는 system clipboard text를 active PTY에 paste한다.
...
- **FR-IN-5 [P0]** 선택 영역이 있을 때 Ctrl-C, Ctrl-Shift-C, 또는 Cmd-Shift-C/Super-Shift-C는 선택 텍스트를 system clipboard에 복사하고 PTY로 interrupt를 보내지 않아야 한다.
- **FR-IN-5a [P0]** 선택 영역이 있을 때 copy command가 실패해도 Ctrl-C interrupt가 child로 누수되면 안 된다.
- **FR-IN-5b [P0]** 선택 영역이 없을 때 Cmd-C/Super-C와 Cmd-Shift-C/Super-Shift-C는 일반 문자 `c`를 child PTY로 보내지 않아야 한다.
- **FR-IN-5c [P0]** 선택 영역이 있을 때 Cmd-Shift-X/Super-Shift-X 또는 Ctrl-Shift-X는 editable selection이면 선택 텍스트를 system clipboard에 복사하고 cursor 이동과 Backspace로 삭제해야 한다. 삭제할 수 없는 출력 화면 selection이면 복사 후 tuimux selection을 해제해야 하며, 선택 영역이 없으면 일반 문자 `x`를 child PTY로 보내지 않아야 한다.
...
- **FR-IN-18 [P0]** terminal mode에서 Ctrl-V, Ctrl-Shift-V, 또는 Cmd-Shift-V/Super-Shift-V는 system clipboard text를 active PTY에 paste해야 한다.
- **FR-IN-19 [P0]** terminal mode에서 Home/End는 active PTY에 line start/end key로 전달되어야 한다.
- **FR-IN-20 [P0]** terminal mode에서 Cmd-Shift-Left/Super-Shift-Left와 Cmd-Shift-Right/Super-Shift-Right는 각각 Home과 End처럼 처리되어야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.38 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
