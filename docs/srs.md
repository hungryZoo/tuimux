...
- 선택 영역이 있을 때 Ctrl-C/Ctrl-Shift-C/Cmd-Shift-C는 child process SIGINT가 아니라 system clipboard copy로 처리된다. Cmd-Shift-X는 선택 텍스트를 clipboard에 복사한 뒤 tuimux selection을 해제한다. 선택 영역이 없으면 plain Ctrl-C는 child process interrupt로 전달된다.
...
- 우클릭은 TUI context menu를 열어 Cut, Copy, Paste, Cancel을 제공한다. Cut은 선택 영역을 system clipboard로 복사한 뒤 tuimux selection을 해제하고, Copy는 선택 영역을 system clipboard로 복사하며, Paste는 system clipboard text를 active PTY에 paste한다.
...
- **FR-IN-5c [P0]** 선택 영역이 있을 때 Cmd-Shift-X/Super-Shift-X 또는 Ctrl-Shift-X는 선택 텍스트를 system clipboard에 복사하고 tuimux selection을 해제해야 하며, 선택 영역이 없으면 일반 문자 `x`를 child PTY로 보내지 않아야 한다.
...
- **FR-IN-15a [P0]** context menu의 Cut은 selection이 있으면 선택 텍스트를 system clipboard로 복사하고 tuimux selection을 해제해야 한다.
...
- **AC-26 [P0]** macOS PTY UI smoke는 실제 `tuimux` TUI에서 SGR mouse drag 후 mouse-up 상태의 reverse-video selection highlight, right-click context menu, Cut/Copy action, Ctrl-C, `pbpaste`, foreground child SIGINT 미전달, Paste action, child bracketed paste wrapper 보존을 검증해야 한다.
