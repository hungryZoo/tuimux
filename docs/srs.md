...
- 선택 영역이 있을 때 Ctrl-C/Cmd-C는 child process SIGINT가 아니라 system clipboard copy로 처리된다. 선택 영역이 없으면 plain Ctrl-C는 child process interrupt로 전달된다.
- Ctrl-V/Cmd-V는 system clipboard text를 active PTY에 paste한다.
...
- **FR-IN-5 [P0]** 선택 영역이 있을 때 Ctrl-C 또는 Cmd-C/Super-C는 선택 텍스트를 system clipboard에 복사하고 PTY로 interrupt를 보내지 않아야 한다.
- **FR-IN-5a [P0]** 선택 영역이 있을 때 copy command가 실패해도 Ctrl-C interrupt가 child로 누수되면 안 된다.
- **FR-IN-5b [P0]** 선택 영역이 없을 때 Cmd-C/Super-C는 일반 문자 `c`를 child PTY로 보내지 않아야 한다.
...
- **FR-IN-18 [P0]** terminal mode에서 Ctrl-V 또는 Cmd-V/Super-V는 system clipboard text를 active PTY에 paste해야 한다.
- **FR-IN-19 [P0]** terminal mode에서 Home/End는 active PTY에 line start/end key로 전달되어야 한다.
- **FR-IN-20 [P0]** terminal mode에서 Cmd-Left/Super-Left와 Cmd-Right/Super-Right는 각각 Home과 End처럼 처리되어야 한다.
...
- **AC-13d [P0]** 선택 영역이 없을 때 plain Ctrl-C는 active child program interrupt로 동작하고, Cmd-C/Super-C는 일반 `c` 입력으로 누수되지 않는다.
- **AC-13e [P0]** Ctrl-V/Cmd-V/Super-V는 system clipboard를 active PTY에 paste한다.
- **AC-13f [P0]** Home/End와 Cmd-Left/Cmd-Right는 active terminal input line의 start/end 이동으로 동작한다.
...
- **4.2 / 2026-06-13**: Ctrl-C/Cmd-C/Ctrl-V/Cmd-V와 Home/End/Cmd-arrow keyboard shortcut 요구사항을 추가하고, 선택 없는 Ctrl-C는 child interrupt로 유지.
