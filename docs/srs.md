...
- 붙여넣기 직후 child shell이 paste highlight를 그려둔 경우 child mouse protocol이 꺼져 있으면 다음 일반 mouse down/up에서 하이라이트가 해제되어야 한다. 우클릭 context menu 요청도 highlight clear 뒤에 처리되어야 한다.
...
- **FR-IN-21 [P0]** paste 성공 또는 raw bracketed paste end sequence 관측 후 child mouse protocol이 꺼진 상태에서 mouse down/up이 발생하면 쉘의 paste highlight pending 상태를 해제해야 하며, 같은 click/drag/context menu 동작은 유지해야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.34 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-13i [P0]** Cut으로 clipboard에 들어간 텍스트를 context menu Paste로 붙인 뒤 right click하면 foreground child가 paste highlight clear용 cursor key 왕복 입력을 받아야 한다.
