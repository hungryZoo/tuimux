...
- 기본 UI에서 split-pane 생성/resize/cycle/kill 제공. 해당 기능은 deprecated이며 default daemon protocol에서도 노출하지 않는다.
...
- **FR-DMN-9 [P0]** split-pane 관련 request는 default product protocol에서 제거하거나 deprecated 상태로 유지해야 하며 UI에서 호출하면 안 된다.
...
- **FR-MUX-9 [P1]** legacy split-pane 구현은 기본 UI/daemon protocol 밖에 격리하고 향후 삭제 가능해야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.11 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-17 [P0]** navigation mode에서 split hotkey는 새 pane을 만들지 않고 deprecated status를 표시한다.
- **AC-18 [P0]** `cargo build --release --locked --target aarch64-apple-darwin`가 성공한다.
- **AC-19 [P0]** macOS ARM installer가 `tuimux --version`과 `tuimux --doctor` 검증 안내를 출력한다.
- **AC-20 [P0]** host paste는 개별 key 입력 폭주가 아니라 paste event로 처리되어 active PTY에 전달된다.
- **AC-21 [P0]** client A가 socket connection을 유지한 상태에서 client B가 snapshot/window/scrollback command를 수행하고, client A가 계속 응답을 받을 수 있다.
- **AC-22 [P0]** client A가 열린 상태에서도 client C의 shutdown request가 daemon을 종료한다.
