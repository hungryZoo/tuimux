...
- 붙여넣기 직후 child shell이 paste highlight를 그려둔 경우 child mouse protocol이 꺼져 있으면 다음 일반 left click에서 하이라이트가 해제되어야 한다.
...
- **FR-IN-21 [P0]** paste 성공 또는 raw bracketed paste end sequence 관측 후 child mouse protocol이 꺼진 상태에서 left click하면 쉘의 paste highlight pending 상태를 해제해야 하며, 같은 click/drag의 tuimux selection 동작은 유지해야 한다.
