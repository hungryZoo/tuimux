...
tuimux는 기본 실행 경로에서 tmux server/client에 의존하지 않는 Rust-native terminal multiplexer다. 사용자가 `tuimux`를 실행하면 UI client는 tuimux daemon에 attach하고, daemon은 단일 mux의 window list, PTY child process, terminal screen model을 직접 소유해야 한다.

tmux의 C 코드는 window/PTY ownership/입력 라우팅/mouse mode 처리의 참고 자료로만 사용한다. 제품 코드는 C tmux 수정이나 1:1 포팅이 아니라 Rust로 직접 구현한다.
...
- UI detach/종료 후에도 daemon-owned window/PTY child process를 유지한다.
- 같은 daemon에 여러 client가 동시에 연결되어 snapshot/명령 요청을 처리할 수 있다.
- Rust-native window 목록, window 생성/선택/종료를 제공한다.
...
- terminal mode는 넓은 화면에서 오른쪽 boxed window/action rail을 제공하되 위/아래 status bar는 만들지 않아야 한다. 좁은 화면의 compact top tab fallback은 btop 같은 full-screen 앱의 PTY 크기 보존을 위해 비활성화한다. `F12` navigation/sidebar mode는 별도로 제공한다.
...
- active input cursor가 보이는 terminal mode에서는 plain Shift+방향키로 text editor와 유사한 keyboard selection을 만들 수 있어야 한다. Shift+Up/Shift+Down은 shell history key가 아니라 terminal body 폭만큼의 좌우 cursor movement로 처리한다.
- 선택 영역이 있을 때 Ctrl-C/Ctrl-Shift-C/Cmd-Shift-C/Win-Shift-C는 child process SIGINT가 아니라 system clipboard copy로 처리된다. Cmd-Shift-X/Win-Shift-X는 editable 선택 텍스트를 clipboard에 복사한 뒤 cursor 이동과 Backspace로 삭제하고, 삭제할 수 없는 출력 화면 선택은 복사 후 tuimux selection을 해제한다. 선택 영역이 없으면 plain Ctrl-C는 child process interrupt로 전달된다.
- editable 선택 영역이 있을 때 Backspace/Delete는 선택 영역을 삭제하고, 일반 문자 입력은 선택 영역을 삭제한 뒤 해당 문자를 입력한다. editable 선택은 soft wrap과 hard newline을 포함한 여러 화면줄 선택을 지원해야 하며, 오른쪽 빈칸까지 드래그해도 실제 텍스트만 삭제해야 한다.
- Ctrl-V/Ctrl-Shift-V/Cmd-Shift-V/Win-Shift-V는 system clipboard text를 active PTY에 paste한다.
- host terminal paste는 crossterm paste event로 받아 active PTY로 전달하되 child bracketed paste mode를 존중한다.
- host terminal paste가 raw `ESC [ 200 ~` / `ESC [ 201 ~` key sequence로 전달되는 경우도 paste highlight clear 대상 paste로 추적한다.
- 붙여넣기 직후 child shell이 paste highlight를 그려둔 경우 terminal body left click은 단순 하이라이트 제거가 아니라 클릭한 terminal cell로 active input cursor를 이동시키는 동작이어야 한다. 이 cursor 이동 결과로 paste highlight가 해제되어야 하며, 같은 mouse down/up이 child mouse escape로 누수되면 안 된다.
...
- child application이 mouse tracking을 켠 경우 simple left click과 wheel은 child로 전달하고, normal drag는 tuimux selection으로 처리한다.
- 우클릭은 TUI context menu를 열어 Cut, Copy, Paste, Cancel을 제공한다. Cut은 editable 선택 영역을 system clipboard로 복사한 뒤 cursor 이동과 Backspace로 삭제하고, 삭제할 수 없는 출력 화면 선택은 복사 후 tuimux selection을 해제한다. Copy는 선택 영역을 system clipboard로 복사하며, Paste는 system clipboard text를 active PTY에 paste한다.
...
- daemon 재시작 후 window 복구를 위한 disk persistence.
- 동시 다중 attach의 독립 cursor/viewport 정책.
- Windows named-pipe daemon backend.
- Linux/Windows 프리릴리즈 asset.

현재 `Detach`는 UI client 종료이며 daemon과 child PTY는 계속 살아남는다. 다만 `tuimux --stop-server`, daemon crash, host reboot 후에는 window 복구를 보장하지 않는다.
...
- **FR-DMN-3 [P0]** socket path는 macOS Unix socket path length 제한을 넘지 않도록 `/tmp/tuimux-$USER/<scope-hash>.sock` 형태의 짧은 경로를 사용해야 한다. scope는 내부 daemon 격리용이며 사용자-facing session 개념이 아니다.
- **FR-DMN-4 [P0]** daemon은 JSON line request/response protocol로 snapshot, key, paste, mouse, window command, scrollback command를 처리해야 한다.
- **FR-DMN-5 [P0]** `--stop-server`는 기본 daemon에 shutdown request를 보내야 한다.
...
- **FR-DMN-8 [P0]** 공유 mux state 변경은 mutex로 직렬화해 window/terminal state corruption을 방지해야 한다.
- **FR-DMN-9 [P0]** split-pane 관련 request는 default product protocol에서 제거해야 하며 UI에서 호출하면 안 된다.

### 3.3 Native Multiplexer

- **FR-MUX-1 [P0]** daemon은 시작 시 단일 mux와 shell window 하나를 직접 생성해야 한다.
- **FR-MUX-2 [P0]** native mux core는 session list, active session index, session 생성/선택 command를 제공하지 않아야 한다.
- **FR-MUX-3 [P0]** window 생성은 단일 window list에 새 PTY-backed shell을 추가해야 한다.
- **FR-MUX-4 [P0]** window 선택은 active window index만 변경하고 host terminal이나 외부 tmux client를 조작하지 않아야 한다.
- **FR-MUX-5 [P0]** window 선택은 active window를 변경하고 해당 window의 screen을 terminal body surface로 렌더해야 한다.
- **FR-MUX-6 [P0]** 마지막 window를 종료하면 replacement shell window를 만들어 빈 mux panic을 방지해야 한다.
- **FR-MUX-7 [P0]** UI detach 후 다시 `tuimux`로 attach하면 기존 shell state가 유지되어야 한다.
...
- **FR-IN-3a [P0]** terminal mode에서도 `Alt-N`은 새 window를 만들고, `Alt-Left`/`Alt-Right`는 active window를 전환해야 한다.
- **FR-IN-4 [P0]** 선택 영역이 없을 때 Ctrl-C는 active PTY로 전달되어 실행 중 프로그램의 일반 Ctrl-C로 동작해야 한다.
- **FR-IN-5 [P0]** 선택 영역이 있을 때 Ctrl-C, Ctrl-Shift-C, 또는 Cmd-Shift-C/Win-Shift-C/Super-Shift-C/META-Shift-C/HYPER-Shift-C는 선택 텍스트를 system clipboard에 복사하고 PTY로 interrupt를 보내지 않아야 한다.
- **FR-IN-5a [P0]** 선택 영역이 있을 때 copy command가 실패해도 Ctrl-C interrupt가 child로 누수되면 안 된다.
- **FR-IN-5b [P0]** 선택 영역이 없을 때 Cmd-C/Win-C/Super-C/META-C/HYPER-C와 Cmd-Shift-C/Win-Shift-C/Super-Shift-C/META-Shift-C/HYPER-Shift-C는 일반 문자 `c`를 child PTY로 보내지 않아야 한다.
- **FR-IN-5c [P0]** 선택 영역이 있을 때 Cmd-Shift-X/Win-Shift-X/Super-Shift-X/META-Shift-X/HYPER-Shift-X 또는 Ctrl-Shift-X는 editable selection이면 선택 텍스트를 system clipboard에 복사하고 cursor 이동과 Backspace로 삭제해야 한다. 삭제할 수 없는 출력 화면 selection이면 복사 후 tuimux selection을 해제해야 하며, 선택 영역이 없으면 일반 문자 `x`를 child PTY로 보내지 않아야 한다.
- **FR-IN-5d [P0]** editable selection이 있을 때 Backspace/Delete는 selection을 삭제하고 원래 Backspace/Delete 입력은 추가로 child에 보내지 않아야 한다.
- **FR-IN-5e [P0]** editable selection이 있을 때 일반 문자 입력은 selection을 삭제한 뒤 해당 문자를 child에 보내 selection replacement처럼 동작해야 한다.
- **FR-IN-5f [P0]** editable selection 삭제는 화면 좌표를 `row * pane_width + col` 선형 좌표로 계산해 soft wrap과 hard newline이 포함된 여러 줄 선택을 지워야 한다. hard newline은 삭제 문자 1개로 계산하고 다음 화면 행의 col 0으로 이동한 것으로 해석해야 하며, selection이 오른쪽 빈칸까지 이어져도 실제 선택 텍스트 뒤의 blank tail은 삭제 범위에 포함하지 않아야 한다.
- **FR-IN-5g [P0]** active input cursor가 보이면 plain Shift+Left/Right/Up/Down은 cursor boundary 기반 keyboard selection을 확장해야 한다. Shift+Left와 Shift+Right는 한 글자 selection을 표현할 수 있어야 하며, Shift+Up/Down은 PTY에 Up/Down history key를 보내지 않고 terminal body 폭만큼 raw left/right cursor movement를 보내야 한다.
...
- **FR-IN-8 [P0]** child가 mouse protocol을 켠 상태에서는 simple left click과 wheel event를 child로 보내야 한다.
- **FR-IN-9 [P0]** child mouse protocol 활성 상태에서도 normal left drag는 tuimux selection을 시작해야 하며 drag sequence가 child로 누수되면 안 된다.
...
- **FR-IN-13 [P0]** child mouse protocol이 켜져 있으면 mouse wheel과 simple left click은 child로 전달해야 한다.
- **FR-IN-14 [P0]** 우클릭 시 TUI context menu가 떠야 하며 항목은 Cut, Copy, Paste, Cancel이어야 한다.
- **FR-IN-15 [P0]** context menu의 Copy는 selection이 있으면 선택 텍스트를 system clipboard로 복사해야 한다.
- **FR-IN-15a [P0]** context menu의 Cut은 editable selection이면 선택 텍스트를 system clipboard로 복사하고 cursor 이동과 Backspace로 삭제해야 한다. 삭제할 수 없는 출력 화면 selection이면 복사 후 tuimux selection을 해제해야 한다.
- **FR-IN-16 [P0]** context menu의 Paste는 selection 유무와 무관하게 system clipboard text를 active PTY에 paste해야 한다.
- **FR-IN-17 [P0]** context menu의 Cancel 또는 `Esc`는 cut/copy/paste 없이 menu를 닫아야 한다.
- **FR-IN-18 [P0]** terminal mode에서 Ctrl-V, Ctrl-Shift-V, 또는 Cmd-Shift-V/Win-Shift-V/Super-Shift-V/META-Shift-V/HYPER-Shift-V는 system clipboard text를 active PTY에 paste해야 한다.
- **FR-IN-19 [P0]** terminal mode에서 Home/End는 active PTY에 line start/end key로 전달되어야 한다.
- **FR-IN-20 [P0]** terminal mode에서 Cmd-Shift-Left/Win-Shift-Left/Super-Shift-Left/META-Shift-Left/HYPER-Shift-Left와 Cmd-Shift-Right/Win-Shift-Right/Super-Shift-Right/META-Shift-Right/HYPER-Shift-Right는 각각 Home과 End처럼 처리되어야 한다.
- **FR-IN-21 [P0]** paste 성공 또는 raw bracketed paste end sequence 관측 후 terminal body left click이 발생하면 현재 cursor cell과 clicked cell의 visual offset을 계산해 active PTY에 필요한 `ESC[D` 또는 `ESC[C` cursor movement를 보내야 한다.
- **FR-IN-21a [P0]** paste 뒤 left click의 성공 기준은 하이라이트 제거가 아니라 active input cursor가 clicked cell로 이동하고, 그 결과 shell-side paste highlight가 해제되는 것이다.
- **FR-IN-21b [P0]** paste cursor move를 수행한 terminal body left click은 같은 click의 mouse down/up이 child로 전달되지 않도록 소비해야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.39 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-4a [P0]** 넓은 기본 terminal mode 화면에는 boxed `Detach`, `WINDOWS`, window rows, 3-cell ` X ` close buttons, `+ new`, `STATUS`, `scroll:<count>`가 보여야 하며 `Session` panel은 렌더링되지 않아야 한다.
- **AC-4b [P0]** terminal mode에서 `STATUS` panel을 클릭하면 active terminal scrollback이 bottom으로 이동해 `scroll:0`이 되어야 한다.
- **AC-5 [P0]** terminal mode에서 `printf 'hello\n'` 입력이 active shell에서 실행된다.
- **AC-6 [P0]** UI를 종료한 뒤 다시 `tuimux`로 attach하면 shell 환경값이 유지된다.
...
- **AC-13d [P0]** 선택 영역이 없을 때 plain Ctrl-C는 active child program interrupt로 동작하고, Cmd-C/Super-C/Cmd-Shift-C/Super-Shift-C는 일반 `c` 입력으로 누수되지 않는다.
- **AC-13e [P0]** Ctrl-V/Ctrl-Shift-V/Cmd-Shift-V/Super-Shift-V는 system clipboard를 active PTY에 paste한다.
- **AC-13f [P0]** Home/End와 Cmd-Shift-Left/Cmd-Shift-Right는 active terminal input line의 start/end 이동으로 동작한다.
- **AC-13a [P0]** 우클릭하면 selection 유무와 무관하게 Cut, Copy, Paste, Cancel을 담은 TUI context menu가 표시된다.
- **AC-13b [P0]** 선택 영역이 있을 때 context menu Cut/Copy 선택 후 macOS `pbpaste`가 선택 텍스트를 반환한다.
- **AC-13d [P0]** editable selection에서 context menu Cut을 선택하면 foreground child가 필요한 cursor 이동과 선택 글자 수만큼 Backspace를 받아야 한다.
- **AC-13g [P0]** editable selection에서 Backspace를 누르면 foreground child가 필요한 cursor 이동과 선택 글자 수만큼 Backspace를 받아야 하며 원래 Backspace가 추가로 누수되면 안 된다.
- **AC-13h [P0]** editable selection에서 일반 문자를 입력하면 foreground child가 selection 삭제 입력 뒤 해당 문자를 받아야 한다.
- **AC-13i [P0]** Copy 또는 Cut으로 clipboard에 들어간 텍스트를 context menu Paste로 붙인 뒤 terminal body left click하면 foreground child가 clicked cell 방향으로 2회 이상의 cursor movement를 받아야 한다.
- **AC-13j [P0]** 실제 zsh prompt에서 bracketed paste 뒤 terminal body left click은 reverse-video paste highlight를 지우고 최종 cursor position을 clicked column 근처로 이동시켜야 하며 mouse escape를 입력줄에 남기지 않아야 한다.
- **AC-13k [P0]** paste 상태가 아닌 실제 zsh prompt에서도 terminal body left click은 selection을 만들지 않고 active input cursor를 clicked column으로 이동해야 하며 mouse escape를 입력줄에 남기지 않아야 한다.
- **AC-13l [P0]** editable selection에서 Ctrl-V/Ctrl-Shift-V/Cmd-Shift-V paste를 수행하면 selection이 먼저 삭제되고 clipboard text가 replacement로 입력되어야 한다.
- **AC-13m [P0]** context menu가 열려 있어도 copy/cut/paste shortcut은 메뉴에 삼켜지지 않고 context menu action과 같은 clipboard/paste 경로를 사용해야 한다.
- **AC-13n [P0]** editable selection에서 Backspace/Delete/일반 문자/Ctrl-V/Ctrl-Shift-X를 수행하면 raw child line editor의 최종 buffer가 각각 `prefix + replacement + suffix` 규칙을 만족해야 한다.
- **AC-13o [P0]** editable selection에서 Ctrl-Shift-X cut shortcut을 수행하면 selection text가 system clipboard에 복사되고 raw child line editor buffer에서는 target이 삭제되어야 한다.
- **AC-13p [P0]** soft wrap된 editable selection을 빈칸까지 드래그한 뒤 일반 문자를 입력하면 target만 replacement로 대체되어야 한다.
- **AC-13q [P0]** hard newline이 포함된 editable selection에서 Backspace를 누르면 newline까지 포함한 target만 삭제되어 raw child line editor buffer가 `prefix + suffix`가 되어야 한다.
- **AC-13c [P0]** context menu Paste 선택 시 macOS clipboard text를 active PTY에 paste한다.
...
- **AC-26 [P0]** macOS PTY UI smoke는 실제 `tuimux` TUI에서 SGR mouse drag 후 mouse-up 상태의 reverse-video selection highlight, right-click context menu, Cut/Copy action, Cut Backspace child 전달, Backspace/delete/text replacement editable selection, Ctrl-C, `pbpaste`, foreground child SIGINT 미전달, Paste action, child bracketed paste wrapper 보존을 검증해야 한다.
- **AC-27 [P0]** macOS window-flow smoke는 실제 `tuimux` TUI에서 navigation mode 진입, 오른쪽 window list 렌더링, `n` 새 window, `x` window 종료, detach, reattach 후 shell 환경값 유지를 검증해야 한다.
...
- **AC-30 [P0]** macOS mouse-protocol smoke는 child가 SGR mouse tracking을 켠 상태에서 normal left click event가 child로 전달되고, normal drag는 tuimux selection으로 처리되어 Ctrl-C clipboard copy가 child에 누수되지 않음을 검증해야 한다.
...
- **AC-43 [P0]** UI regression test와 macOS PTY UI smoke는 paste 성공 후 highlight pending 상태가 세워지고 다음 terminal click clear 경로에서 해제됨을 검증해야 한다.

## 6. 변경 이력

- **4.10 / 2026-06-14**: editable selection 삭제를 soft wrap과 hard newline을 포함한 여러 줄 텍스트 편집 동작으로 확장하고, blank drag tail 제외 및 hard multi-line Backspace smoke 요구사항을 추가.
- **4.9 / 2026-06-14**: copy/cut/paste/click/drag/right-click interaction을 pending left-down과 unified `paste_text()` 경로로 재정의하고, Ctrl-V 실행 및 editable selection replacement smoke 검증을 추가.
- **4.8 / 2026-06-14**: paste 뒤 terminal body left click을 하이라이트 제거가 아니라 clicked cell로 active input cursor를 이동하는 동작으로 재정의하고 zsh visual cursor-position 검증을 추가.
- **4.7 / 2026-06-14**: paste highlight clear를 raw `ESC[D ESC[C`로 고정하고 child mouse protocol/application cursor 상태에서도 terminal body left click이 clear 전용으로 동작하도록 요구사항과 smoke 검증을 갱신.
- **4.6 / 2026-06-14**: Copy/Cut clipboard context paste 뒤 right/left click도 paste highlight를 해제하도록 mouse down/up 선처리 요구사항과 smoke 검증을 추가.
- **4.5 / 2026-06-13**: context menu Cut과 Cmd-Shift-X/Super-Shift-X host cut shortcut 요구사항을 추가.
- **4.4 / 2026-06-13**: paste 후 일반 left click이 쉘 paste highlight를 해제하는 요구사항과 regression test를 추가.
- **4.3 / 2026-06-13**: macOS keyboard shortcut을 Cmd 단독이 아니라 Cmd-Shift-C/V/Left/Right로 조정하고 paste 시 selection 해제를 명시.
- **4.2 / 2026-06-13**: Ctrl-C/Ctrl-V, macOS Cmd-Shift-C/V, Home/End, Cmd-Shift-arrow keyboard shortcut 요구사항을 추가하고, 선택 없는 Ctrl-C는 child interrupt로 유지.
- **4.1 / 2026-06-13**: 우클릭이 즉시 copy/paste하지 않고 TUI context menu를 표시하도록 요구사항을 갱신.
- **4.0 / 2026-06-13**: child mouse tracking 중 simple left click/wheel은 유지하면서 normal drag selection과 right-click copy/paste 요구사항을 추가.
- **3.9 / 2026-06-13**: multi-session 제품 개념과 Session panel/picker를 제거하고 단일 mux window-list 모델을 명시.
...
- **2.3 / 2026-06-13**: macOS window-flow smoke에 실제 TUI detach/reattach와 window-list workflow 검증을 추가하고 alpha.18 요구사항을 추가.
