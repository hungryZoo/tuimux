...
- 붙여넣기 직후 child shell이 paste highlight를 그려둔 경우 child mouse protocol이 꺼져 있으면 다음 일반 mouse down에서 하이라이트가 해제되어야 한다. 우클릭 context menu 요청도 highlight clear 뒤에 처리되어야 한다.
...
- **FR-IN-21 [P0]** paste 성공 또는 raw bracketed paste end sequence 관측 후 child mouse protocol이 꺼진 상태에서 mouse down이 발생하면 쉘의 paste highlight pending 상태를 해제해야 하며, 같은 click/drag/context menu 동작은 유지해야 한다.
...
- **4.6 / 2026-06-14**: Cut clipboard context paste 뒤 right click도 paste highlight를 해제하도록 mouse down 선처리 요구사항과 smoke 검증을 추가.
