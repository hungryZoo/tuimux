...
- 기본 제품 UX는 한 화면을 여러 pane으로 나누지 않고, terminal mode의 오른쪽 통합 rail 또는 compact tab strip, navigation mode의 오른쪽 window 목록에서 active terminal window를 선택하는 방식으로 동작한다.
- 각 window는 실제 PTY-backed shell을 실행한다.
- `vt100` parser 기반 screen model을 ratatui로 렌더링한다.
- terminal mode는 넓은 화면에서 항상 보이는 오른쪽 session/window/action rail과 bottom command/status strip을 제공하고, 좁은 화면에서는 compact top tab strip으로 축약해야 한다. `F12` navigation/sidebar mode는 별도로 제공한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.31 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-4a [P0]** 기본 terminal mode 화면에는 `tuimux` chrome, active session/window 정보, `WINDOWS` rail 또는 compact window tabs, `+ new`, command/status hint가 보여야 한다.
...
- **AC-42 [P0]** macOS terminal-chrome smoke는 실제 `tuimux` TUI 기본 화면에서 integrated right rail, bottom command/status strip, child terminal body 실행, rail `+ new` mouse click, `F12` navigation handoff를 검증해야 한다.
