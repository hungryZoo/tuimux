...
- **FR-DMN-8 [P0]** 공유 mux state 변경은 mutex로 직렬화해 pane/session state corruption을 방지해야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.10 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-22 [P0]** client A가 socket connection을 유지한 상태에서 client B가 snapshot과 split command를 수행하고, client A가 변경된 pane state를 확인할 수 있다.
