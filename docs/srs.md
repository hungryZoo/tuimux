...
- terminal mode는 넓은 화면에서 오른쪽 session/window/action rail을 제공하되 위/아래 status bar는 만들지 않아야 한다. 좁은 화면의 compact top tab fallback은 btop 호환성 확인 전까지 비활성화한다. `F12` navigation/sidebar mode는 별도로 제공한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.32 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-4a [P0]** 넓은 기본 terminal mode 화면에는 borderless `Session`, `Detach`, `WINDOWS`, window rows, `+ new`, scrollback count, hint/status row가 보여야 한다.
...
- **AC-42 [P0]** macOS terminal-chrome smoke는 실제 `tuimux` TUI 기본 화면에서 borderless right rail, scrollback/hint rows, child terminal body 실행, rail `+ new` mouse click, `F12` navigation handoff를 검증해야 한다.
