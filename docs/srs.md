...
- pane resize, nested pane tree, tmux layout string 호환성.
...
- **FR-IN-11 [P0]** split pane에서 mouse event 좌표는 해당 pane의 local row/col로 변환되어 active pane에 전달되어야 한다.
- **FR-IN-12 [P0]** drag selection은 시작 pane을 기억해야 하며, drag가 separator나 다른 pane 위로 지나가도 focus가 튀지 않아야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.7 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
