...
- terminal body 우클릭은 선택 영역이 있으면 system clipboard copy, 선택 영역이 없으면 system clipboard paste로 처리한다.
...
- **FR-IN-14 [P0]** terminal body 우클릭 시 selection이 있으면 선택 텍스트를 system clipboard로 복사하고, selection이 없으면 system clipboard text를 active PTY에 paste해야 한다.
...
- **AC-13a [P0]** 선택 영역이 있을 때 terminal body 우클릭 후 macOS `pbpaste`가 선택 텍스트를 반환한다.
- **AC-13b [P0]** 선택 영역이 없을 때 terminal body 우클릭은 macOS clipboard text를 active PTY에 paste한다.
...
- **AC-26 [P0]** macOS PTY UI smoke는 실제 `tuimux` TUI에서 SGR mouse drag 후 mouse-up 상태의 reverse-video selection highlight, right-click copy, Ctrl-C, `pbpaste`, foreground child SIGINT 미전달, right-click paste, child bracketed paste wrapper 보존을 검증해야 한다.
...
- **4.0 / 2026-06-13**: child mouse tracking 중 simple left click/wheel은 유지하면서 normal drag selection과 terminal body right-click copy/paste 요구사항을 추가.
