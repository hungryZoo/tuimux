...
- child application의 OSC 52 clipboard copy 요청은 system clipboard에 best-effort로 반영한다.
...
- **FR-CLI-7a [P1]** `--layout-preview`는 split-pane/resize 예시가 아니라 single full-size terminal과 오른쪽 window list 중심 preview를 출력해야 한다.
...
- **FR-TERM-16 [P1]** `vt100::Parser` callback으로 OSC 0/1/2 title과 OSC 52 clipboard sequence를 처리해야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.28 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
