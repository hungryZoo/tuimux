...
- 선택 영역이 있을 때 Ctrl-C/Ctrl-Shift-C/Cmd-Shift-C는 child process SIGINT가 아니라 system clipboard copy로 처리된다. Cmd-Shift-X는 현재 입력줄 선택 텍스트를 clipboard에 복사한 뒤 cursor 이동과 Backspace로 삭제하고, 삭제할 수 없는 출력 화면 선택은 복사 후 tuimux selection을 해제한다. 선택 영역이 없으면 plain Ctrl-C는 child process interrupt로 전달된다.
- 현재 입력줄 선택 영역이 있을 때 Backspace/Delete는 선택 영역을 삭제하고, 일반 문자 입력은 선택 영역을 삭제한 뒤 해당 문자를 입력한다.
...
- 우클릭은 TUI context menu를 열어 Cut, Copy, Paste, Cancel을 제공한다. Cut은 현재 입력줄 선택 영역을 system clipboard로 복사한 뒤 cursor 이동과 Backspace로 삭제하고, 삭제할 수 없는 출력 화면 선택은 복사 후 tuimux selection을 해제한다. Copy는 선택 영역을 system clipboard로 복사하며, Paste는 system clipboard text를 active PTY에 paste한다.
...
- **FR-IN-5c [P0]** 선택 영역이 있을 때 Cmd-Shift-X/Super-Shift-X 또는 Ctrl-Shift-X는 현재 입력줄 selection이면 선택 텍스트를 system clipboard에 복사하고 cursor 이동과 Backspace로 삭제해야 한다. 삭제할 수 없는 출력 화면 selection이면 복사 후 tuimux selection을 해제해야 하며, 선택 영역이 없으면 일반 문자 `x`를 child PTY로 보내지 않아야 한다.
- **FR-IN-5d [P0]** 현재 입력줄 selection이 있을 때 Backspace/Delete는 selection을 삭제하고 원래 Backspace/Delete 입력은 추가로 child에 보내지 않아야 한다.
- **FR-IN-5e [P0]** 현재 입력줄 selection이 있을 때 일반 문자 입력은 selection을 삭제한 뒤 해당 문자를 child에 보내 selection replacement처럼 동작해야 한다.
...
- **FR-IN-15a [P0]** context menu의 Cut은 현재 입력줄 selection이면 선택 텍스트를 system clipboard로 복사하고 cursor 이동과 Backspace로 삭제해야 한다. 삭제할 수 없는 출력 화면 selection이면 복사 후 tuimux selection을 해제해야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.37 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-13d [P0]** 현재 입력줄 selection에서 context menu Cut을 선택하면 foreground child가 필요한 cursor 이동과 선택 글자 수만큼 Backspace를 받아야 한다.
- **AC-13g [P0]** 현재 입력줄 selection에서 Backspace를 누르면 foreground child가 필요한 cursor 이동과 선택 글자 수만큼 Backspace를 받아야 하며 원래 Backspace가 추가로 누수되면 안 된다.
- **AC-13h [P0]** 현재 입력줄 selection에서 일반 문자를 입력하면 foreground child가 selection 삭제 입력 뒤 해당 문자를 받아야 한다.
...
- **AC-13l [P0]** 현재 입력줄 selection에서 Ctrl-V/Ctrl-Shift-V/Cmd-Shift-V paste를 수행하면 selection이 먼저 삭제되고 clipboard text가 replacement로 입력되어야 한다.
- **AC-13m [P0]** context menu가 열려 있어도 copy/cut/paste shortcut은 메뉴에 삼켜지지 않고 context menu action과 같은 clipboard/paste 경로를 사용해야 한다.
- **AC-13n [P0]** 현재 입력줄 selection에서 Backspace/Delete/일반 문자/Ctrl-V/Ctrl-Shift-X를 수행하면 raw child line editor의 최종 buffer가 각각 `prefix + replacement + suffix` 규칙을 만족해야 한다.
- **AC-13o [P0]** 현재 입력줄 selection에서 Ctrl-Shift-X cut shortcut을 수행하면 selection text가 system clipboard에 복사되고 raw child line editor buffer에서는 target이 삭제되어야 한다.
