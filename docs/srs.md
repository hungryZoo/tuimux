...
- 선택 영역이 있을 때 Ctrl-C/Ctrl-Shift-C/Cmd-Shift-C는 child process SIGINT가 아니라 system clipboard copy로 처리된다. 선택 영역이 없으면 plain Ctrl-C는 child process interrupt로 전달된다.
...
- 우클릭은 TUI context menu를 열어 Copy, Paste, Cancel을 제공한다. Copy는 선택 영역을 system clipboard로 복사하고, Paste는 system clipboard text를 active PTY에 paste한다.
...
- **FR-IN-14 [P0]** 우클릭 시 TUI context menu가 떠야 하며 항목은 Copy, Paste, Cancel이어야 한다.
...
- **FR-IN-17 [P0]** context menu의 Cancel 또는 `Esc`는 copy/paste 없이 menu를 닫아야 한다.
...
- **AC-13a [P0]** 우클릭하면 selection 유무와 무관하게 Copy, Paste, Cancel을 담은 TUI context menu가 표시된다.
- **AC-13b [P0]** 선택 영역이 있을 때 context menu Copy 선택 후 macOS `pbpaste`가 선택 텍스트를 반환한다.
...
- **AC-26 [P0]** macOS PTY UI smoke는 실제 `tuimux` TUI에서 SGR mouse drag 후 mouse-up 상태의 reverse-video selection highlight, right-click context menu, Copy action, Ctrl-C, `pbpaste`, foreground child SIGINT 미전달, Paste action, child bracketed paste wrapper 보존을 검증해야 한다.
...
- **4.1 / 2026-06-13**: 우클릭이 즉시 copy/paste하지 않고 Copy/Paste/Cancel TUI context menu를 표시하도록 요구사항을 갱신.
