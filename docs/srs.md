...
- 붙여넣기 직후 child shell이 paste highlight를 그려둔 경우 child mouse protocol/application cursor 상태와 무관하게 다음 일반 mouse down/up에서 하이라이트가 해제되어야 한다. terminal body left click은 clear 전용으로 소비하고, 우클릭 context menu 요청은 highlight clear 뒤에 처리되어야 한다.
...
- **FR-IN-21 [P0]** paste 성공 또는 raw bracketed paste end sequence 관측 후 mouse down/up이 발생하면 child mouse protocol/application cursor 상태와 무관하게 active PTY에 raw `ESC[D ESC[C`를 보내 쉘의 paste highlight pending 상태를 해제해야 한다.
- **FR-IN-21a [P0]** paste highlight clear는 우클릭 context menu mouse/request보다 먼저 실행되어야 한다.
- **FR-IN-21b [P0]** paste highlight clear를 수행한 terminal body left click은 같은 click의 mouse down/up이 child로 전달되지 않도록 clear 전용으로 소비해야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.35 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-13i [P0]** Copy 또는 Cut으로 clipboard에 들어간 텍스트를 context menu Paste로 붙인 뒤 click하면 foreground child가 paste highlight clear용 raw `ESC[D ESC[C`를 받아야 한다.
- **AC-13j [P0]** child가 mouse protocol/application cursor mode를 켠 상태에서도 paste 뒤 terminal body left click은 paste highlight를 해제하고 mouse escape를 입력줄에 남기지 않아야 한다.
