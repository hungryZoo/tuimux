...
- 기본 제품 UX는 한 화면을 여러 pane으로 나누지 않고, terminal mode의 상단 session/window tab strip과 navigation mode의 오른쪽 window 목록에서 active terminal window를 선택하는 방식으로 동작한다.
- 각 window는 실제 PTY-backed shell을 실행한다.
- `vt100` parser 기반 screen model을 ratatui로 렌더링한다.
- terminal mode는 항상 보이는 tuimux top tab strip과 bottom command/status strip을 제공하고, `F12` navigation/sidebar mode를 별도로 제공한다.
...
- **FR-TERM-7 [P0]** terminal mode에서는 상단 tuimux tab strip과 하단 command/status strip을 제외한 terminal body를 active child PTY 크기로 사용해야 한다.
...
- **FR-REL-1 [P0]** v0.2.0-alpha.30 프리릴리즈는 macOS Apple Silicon tarball만 게시한다.
...
- **AC-4a [P0]** 기본 terminal mode 화면에는 `tuimux` top chrome, active session/window tab, `F12 nav`, `Alt-N new` command hint가 보여야 한다.
- **AC-5 [P0]** terminal mode에서 `printf 'hello\n'` 입력이 active shell에서 실행된다.
- **AC-6 [P0]** UI를 종료한 뒤 같은 `--session`으로 reattach하면 shell 환경값이 유지된다.
- **AC-7 [P0]** `btop`이 32x120 host에서 visible tuimux chrome을 제외한 terminal body 안에 “terminal too small” 오류 없이 열린다.
...
- **AC-27 [P0]** macOS session-flow smoke는 실제 `tuimux` TUI에서 navigation mode 진입, 오른쪽 window list 렌더링, `n` 새 window, split hotkey deprecated status, `x` window 종료, detach, 같은 session reattach 후 shell 환경값 유지를 검증해야 한다.
...
- **AC-35 [P0]** macOS child-exit smoke는 마지막 shell의 `exit` 후 replacement shell이 새 명령을 받을 수 있고, non-last window의 shell `exit` 후 해당 window가 오른쪽 window list에서 제거됨을 검증해야 한다.
...
- **AC-42 [P0]** macOS terminal-chrome smoke는 실제 `tuimux` TUI 기본 화면에서 top tab strip, bottom command/status strip, child terminal body 실행, `Alt-N` 새 window, `F12` navigation handoff를 검증해야 한다.

## 6. 변경 이력

- **3.5 / 2026-06-13**: 기본 terminal mode에 항상 보이는 tuimux top/bottom chrome, terminal-mode hotkey, macOS terminal-chrome smoke를 추가해 alpha.30 요구사항을 추가.
...
- **2.3 / 2026-06-13**: macOS session-flow smoke에 실제 TUI detach/reattach, window-list workflow, split deprecated 검증을 추가하고 alpha.18 요구사항을 추가.
