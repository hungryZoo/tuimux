...
- `--layout-preview` static preview.
- `--doctor` 환경 체크.
- macOS prerelease packaging/installer.

### 제외

- full tmux control-mode streaming client.
- 좌측 파일 탐색기.
- 하단 메뉴 바.
- PROCS panel.
...
- **FR-DETACH-5 [M]** Detach button 클릭 또는 `d`는 detach exit path로 종료한다.
...
- **FR-DIALOG-6 [M]** dialog 하단에 `New Session` button과 빨간 Detach button을 둔다.
- **FR-DIALOG-7 [M]** dialog action button들은 테두리 title/header 없이 버튼 내부 텍스트만 표시한다.
...
- **FR-DIALOG-12 [M]** dialog가 열려 있을 때 `n` 키 또는 `New Session` 클릭은 `tuimux-<n>` 이름의 detached session을 생성하고 live state를 refresh한다.
...
- **AC-2** `cargo run -- --layout-preview` 출력에 `[ Session ]`, `[ New Session ]`, `[ Detach ]`, `WINDOWS`, `✕`, `● dev`가 있고, `MAIN AREA`/`mock`/`Session picker`/`Sessions` 헤더가 없다.
...
- PRD 기능 검증 → SRS FR-CLI, FR-WIN, FR-DIALOG, AC-1~AC-7.
