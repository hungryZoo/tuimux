...
- In terminal mode, `Home`/`End` and macOS `Cmd-Left`/`Cmd-Right` move to the start/end of the active terminal input line.
...
- `Ctrl-C`/`Cmd-C` copy only when a drag selection exists; without a selection, plain `Ctrl-C` still reaches the child program as SIGINT. `Ctrl-V`/`Cmd-V` paste the system clipboard into the active PTY.
