...
v0.2.0-alpha.10 keeps the default runtime on the Rust-native path and adds
daemon-owned nested split panes, pane resize, concurrent daemon clients, and host bracketed paste handling. Running `tuimux` now attaches a ratatui
...
Pushing a tag like `v0.2.0-alpha.10` triggers `.github/workflows/release.yml`,
